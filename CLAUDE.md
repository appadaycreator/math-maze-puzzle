...
#### P1-1: SNS共有機能の実装
- ゲーム結果をTwitter/Facebook/LINE/URLコピーで共有可能
- 共有テキスト自動生成：「スコア・難易度・正答率」を含む
- イベントリスナー設定で各SNSプラットフォームのシェアURLを動的生成
- window.openでSNS投稿ページをポップアップ表示

#### P1-2: 正答フィードバックの視覚的改善
- CSS @keyframes アニメーション追加：
  - `correctFlash`：正解時に緑色フラッシュ
  - `wrongShake`：不正解時にシェイク＋赤フラッシュ
  - `comboFloat`：コンボ表示の浮遊アニメーション
- JavaScript の showAnswerFeedback メソッド改良：
  - 背景フラッシュのアニメーション追加
  - タイミング自動管理（800ms/500ms）

#### P2-1: 難易度による点数ボーナス機能
- GAME_CONFIG に DIFFICULTY_MULTIPLIERS 追加：
  - 初級：1.0倍（10点）
  - 中級：1.5倍（15点）
  - 上級：2.0倍（20点）
  - マスター：2.5倍（25点）
- handleCorrectAnswer メソッドを改良：
  - 難易度乗数を自動適用
  - 連続正解ボーナスも難易度に応じて計算
